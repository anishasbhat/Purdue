...
public class Part3Test {
    ...
    public static void main(String[] args) throws FileNotFoundException { //added
    	//added
	...
	Patient p = list.remove(i);
		System.out.println(p.posInQueue() + " " + p.urgency());
	pq.update(p.name(), 100);
		System.out.println(p.posInQueue() + " " + p.urgency());
	Patient s = pq.delMax();
		System.out.println(p.posInQueue() + " vs " + s.posInQueue() + " " + s.urgency());
	...
	//System.out.println(p2 + " " + p.posInQueue());
	...
	for(int j = 0; j < 10; j++) {
		...
		//System.out.println("valid after remove several?:" + checkPosInQueue(pq));
	if(pass) {
	...
	int[] diffs = new int[] {-3, -2, -1, 1, 2, 3};
		Patient[] arry = pq.getArray();
		for (int j = 0; j < list.size(); j++) {
			System.out.println(list.get(j) + " vs " + arry[j]);
		}
	for(int j = 0; j < 10; j++) {
	    i = gen.nextInt(list.size());
	...
	pass = checkPosInQueue(pq);
		...
		for (int j = 0; j < list.size(); j++) {
			System.out.println(list.get(j) + " vs " + array[j]);
		}
	if(pass) {
	...
	while(!list.isEmpty()) {
		...
		System.out.println(pExp + " vs " + pAct);

	    if(pExp != pAct || pAct.posInQueue() != -1) {
			Patient[] arr = pq.getArray();
			for (int j = 0; j < list.size(); j++) {
				System.out.println(list.get(j) + " vs " + arr[j]);
			}
	...
	for(int i = 0; i < array.length; i++) {
		//System.out.println(i + " versus " + array[i].posInQueue());
	    if(array[i] != null) {
