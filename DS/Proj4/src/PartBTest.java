...
public class PartBTest 
{
    ...
    private static void runTest(int testNum, int graphNum, int numReg, int firstReg, String[] interReg) 
    {
	    ...
	    Graph O = new Graph("/Users/anishasbhat/Downloads/CS251/Proj4/src/test/testFiles/graph" + graphNum + ".txt");
	    Graph[] regions = new Graph[numReg];
	    int j = 0;
	    
        while(j < numReg) 
        {
	        regions[j] = new Graph("/Users/anishasbhat/Downloads/CS251/Proj4/src/test/testFiles/region" + (firstReg + j) + ".txt");
	        j++;
	    }
	    
        Graph G = GlobalNet.run(O, regions);
	    Graph E = new Graph("/Users/anishasbhat/Downloads/CS251/Proj4/src/test/testFiles/B_test_" + testNum +".txt");
